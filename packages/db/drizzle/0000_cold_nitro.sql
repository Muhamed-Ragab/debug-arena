...
CREATE POLICY "user_own_profile_links" ON "profile_links" AS PERMISSIVE FOR ALL TO "user" USING ("profile_links"."user_id" = (select current_setting('request.jwt.claim.sub')::uuid)) WITH CHECK ("profile_links"."user_id" = (select current_setting('request.jwt.claim.sub')::uuid));--> statement-breakpoint
...
CREATE POLICY "user_own_profile" ON "users" AS PERMISSIVE FOR ALL TO "user" USING ("users"."id" = (select current_setting('request.jwt.claim.sub')::uuid)) WITH CHECK ("users"."id" = (select current_setting('request.jwt.claim.sub')::uuid));