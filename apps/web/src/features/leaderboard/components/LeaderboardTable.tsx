        ...
        <tr>
          {["Rank", "User", "Score", "Solved", "Streak"].map((col) => (
            <th
              key={col}
              className={cn(
                "text-left text-[11px] text-zinc-700 font-medium uppercase tracking-wider pb-3 font-mono",
                (col === "Score" || col === "Solved" || col === "Streak") && "text-right",
              )}
            >
              {col}
            </th>
            ...
            <td className="py-3">
              <div className="flex items-center gap-2.5">
                ...
                >
                  {row.name}
                  {row.isUser && (
                    <span className="ml-2 text-[10px] text-indigo-500 font-mono bg-indigo-500/10 px-1.5 py-0.5 rounded">
                      you
                    </span>
                  )}
                </span>
