...
import { calculateUserRank } from "@/features/leaderboard/service";
...
import type { SubmitChallengeInput } from "./types";
...
export async function getPublishedChallengesMapped(repo = challengeRepository) {
...
export async function getChallengeByIdMapped(
  id: string,
  repo = challengeRepository
) {
...
export async function getUserChallengeStats(
  ...
  const data = await challengeRepo.findUserChallengeStatsData(userId ?? "");
  const totalCount = data.totalPublishedCount;
  if (!(userId && data.user)) {
    return { rank: "#--", solvedRatio: `0 / ${totalCount}`, streak: "0 days" };
  }
...
async function updateUserRatingAndStreak(userId: string, totalScore: number) {
  const data = await challengeRepository.findUserChallengeStatsData(userId);
  const { user } = data;
  if (!user) {
    return;
  }
  ...
  // Use profileRepository to update
...
export async function submitChallenge(
  ...
  const localizationAnswer = formatLocalizationAnswer(input.localizationLines);
  const evaluationDetails = {
