import {
  FIX_QUALITY_LEVEL_MAP,
  ROOT_CAUSE_LEVEL_MAP,
  SENTENCE_SPLIT_REGEX,
} from "./constants";
import type {
  ResultsViewModel,
  ScorePart,
  SubmissionDataForScoring,
} from "./types";

export function getRootCauseDesc(score: number | null): string {
  const s = score ?? 0;
  switch (true) {
    case s >= 20:
      return ROOT_CAUSE_LEVEL_MAP.high;
    case s >= 12:
      return ROOT_CAUSE_LEVEL_MAP.mid;
    default:
      return ROOT_CAUSE_LEVEL_MAP.low;
  }
}

export function getFixQualityDesc(score: number): string {
  switch (true) {
    case score >= 20:
      return FIX_QUALITY_LEVEL_MAP.high;
    case score >= 12:
      return FIX_QUALITY_LEVEL_MAP.mid;
    default:
      return FIX_QUALITY_LEVEL_MAP.low;
  }
}

export function getDynamicAiFeedback(
...
export function computeSubmissionScores(submission: SubmissionDataForScoring) {
...
export function getPreventionNotes(
  canonicalPreventionNotes: string | null | undefined
): string[] {
  if (canonicalPreventionNotes) {
    const parts = canonicalPreventionNotes
...
export function buildEvaluationDetails(
  submission: SubmissionDataForScoring,
  total: number
) {
  const details = submission.evaluationDetails as
...
export function buildResultsViewModel(
  ...
  return {
    aiFeedback: getDynamicAiFeedback(
      submission.aiFeedback as string | null | undefined,
    ...
    evaluationDetails: buildEvaluationDetails(submission, total),
    maxScore: 100,
    preventionNotes: getPreventionNotes(submission.canonicalPreventionNotes),
    scoreParts,
    totalScore: total,
    userExplanation:
